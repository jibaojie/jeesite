...
import com.baojie.jeesite.common.dao.QrtzJobDetailsMapper;
import com.baojie.jeesite.common.exception.DynamicQuartzException;
import com.baojie.jeesite.common.job.TestJob;
...
//		this.checkServiceAndMethod(qrtzJobDetails.getJobName());
		...
		// 构建job信息
		Class jobClass = Class.forName(qrtzJobDetails.getJobClassName());
		JobDetail job = JobBuilder.newJob(jobClass).withIdentity(jobKey).withDescription(qrtzJobDetails.getDescription()).build();
